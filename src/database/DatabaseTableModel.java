...
public class DatabaseTableModel extends AbstractTableModel
{
	...
	public Object getValueAt(int row, int column)
	{
		...
		{
			System.out.println("Failed to get value from table " + table.getName() + ".");
		}
	...
	{
		DataType dataType = DatabaseManager.getSQLType(table.getTableColumns()[columnIndex].getType());
			...
			if (valueParameter != null)
			{
				if (valueParameter.isSetValue())
				{
					if (dataType == DataType.Integer)
					{
						...
						if (intVal < valueParameter.getMinValue() || intVal > valueParameter.getMaxValue())
							success = false;
					}
					else
						System.out.println("Non-Integer values should not have set value.");
				}
				if (valueParameter.isSetLength())
				{
					int length = value.toString().length();

					if (length < valueParameter.getMinValueLength() || length > valueParameter.getMaxValueLength())
						success = false;
				}
			}
		}
		...
		if (success)
			new Thread(new UpdateDatabaseItemRunnable(columnIndex, rowIndex, value, resultSet, dataType)).start();
	}
